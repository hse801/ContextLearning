...
class SSPP(nn.Module):
    ...
    def forward(self, x):
        """
        SSPP2 x1 = torch.Size([1, 128, 10, 16, 20])
        SSPP2 x2 = torch.Size([1, 128, 5, 8, 10])
        SSPP2 x3 = torch.Size([1, 128, 2, 4, 5])
        af SSPP2 x4 = torch.Size([1, 128, 20, 32, 40])
        SSPP2 x1 = torch.Size([1, 128, 20, 32, 40])
        SSPP2 x2 = torch.Size([1, 128, 20, 32, 40])
        SSPP2 x3 = torch.Size([1, 128, 20, 32, 40])
...
class CoConNet(nn.Module):
    ...
    def forward(self, x_list):
        ...
        seg = self.sspp1(seg_x1)
        seg = F.interpolate(seg, size=(int(self.shape[0] / 1), int(self.shape[1] / 1), int(self.shape[2] / 1)), mode='trilinear', align_corners=True)
...
def ASPP_SSPP(shape, num_classes=2, weight_std=True):
    """
    :param shape:
    :param num_classes:
    :param weight_std:
    ...
    ASPP at the end of the encoder, SSPP at the end of the decoder
