...
class SSPP(nn.Module):
    def __init__(self, in_dims, out_dims, shape):
        super(SSPP, self).__init__()
        self.shape = shape
        self.sspp_block1 = nn.Sequential(
            nn.AvgPool3d(3),
            nn.Conv3d(in_dims, out_dims, kernel_size=(1, 1, 1), stride=(1, 1, 1)),
            nn.PReLU(),
            nn.GroupNorm(8, out_dims)
        )
        self.sspp_block2 = nn.Sequential(
            nn.AvgPool3d(3),
            nn.Conv3d(in_dims, out_dims, kernel_size=(2, 2, 2), stride=(2, 2, 2)),
            nn.PReLU(),
            nn.GroupNorm(8, out_dims)
        )
        self.sspp_block3 = nn.Sequential(
            nn.AvgPool3d(3),
            nn.Conv3d(in_dims, out_dims, kernel_size=(8, 8, 8), stride=(8, 8, 8)),
            nn.PReLU(),
            nn.GroupNorm(8, out_dims)
        )
    ...
    def forward(self, x):
        ...
        # print(f'bf x1 = {x1.size()}')
        x1 = F.interpolate(x1, size=(int(self.shape[0] / 1), int(self.shape[1] / 1), int(self.shape[2] / 1)),
                      mode='trilinear', align_corners=True)
        # print(f'af x1 = {x1.size()}')

        x2 = self.sspp_block2(x)
        # print(f'bf x2 = {x2.size()}')
        x2 = F.interpolate(x2, size=(int(self.shape[0] / 1), int(self.shape[1] / 1), int(self.shape[2] / 1)),
                      mode='trilinear', align_corners=True)
        # print(f'af x2 = {x2.size()}')

        x3 = self.sspp_block3(x)
        x3 = F.interpolate(x3, size=(int(self.shape[0] / 1), int(self.shape[1] / 1), int(self.shape[2] / 1)),
                      mode='trilinear', align_corners=True)

        # x4 = self.upsample(x)
        x4 = self.conv_block(x)
        x4 = F.interpolate(x4, size=(int(self.shape[0] / 1), int(self.shape[1] / 1), int(self.shape[2] / 1)),
                      mode='trilinear', align_corners=True)
...
class SSPP2(nn.Module):
    def __init__(self, in_dims, out_dims, shape, stage):
        super(SSPP2, self).__init__()
        self.shape = shape
        self.stage = stage
        self.sspp_block1 = nn.Sequential(
            nn.AvgPool3d(3),
...
class CoConNet(nn.Module):
    def __init__(self, shape, block, layers, num_filters=24, in_channels=1, num_classes=2, weight_std=False):
        ...
        self.sspp1 = nn.Sequential(
            SSPP2(self.num_filters, self.num_filters * 2, self.shape, stage=2)
        )
        self.sspp2 = nn.Sequential(
            SSPP2(self.num_filters, self.num_filters * 2, self.shape, stage=2)
        )
        self.sspp3 = nn.Sequential(
            SSPP2(self.num_filters * 8, self.num_filters * 4, self.shape, stage=4)
        )
        self.sspp4 = nn.Sequential(
            SSPP2(self.num_filters * 8, self.num_filters * 4, self.shape, stage=4)
        )
        self.sspp5 = nn.Sequential(
            SSPP2(self.num_filters * 8, self.num_filters * 4, self.shape, stage=8)
        )
        self.sspp6 = nn.Sequential(
            SSPP2(self.num_filters * 8, self.num_filters * 4, self.shape, stage=8)
        )
    ...
    def forward(self, x_list):
        ...
        x_0_skip = self.sspp3(x_0)
        x_1_skip = self.sspp4(x_1)
        skip2 = torch.cat([x_0_skip, x_1_skip], dim=1)
        ...
        x_0_skip = self.sspp5(x_0)
        x_1_skip = self.sspp6(x_1)
        skip3 = torch.cat([x_0_skip, x_1_skip], dim=1)
        ...
        # print(f'bf sspp x_0 = {x_0.size()}, x_1 = {x_1.size()}')
        x_0 = self.sspp1(x_0)
        x_1 = self.sspp2(x_1)
        # print(f'af sspp x_0 = {x_0.size()}, x_1 = {x_1.size()}')
...
def Co_Con_SSPP_res(shape, num_classes=2, weight_std=True):
