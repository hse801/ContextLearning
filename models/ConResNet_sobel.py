...
class Conv3d(nn.Conv3d):
    ...
    def forward(self, x):
        weight = self.weight
        # print(f'ConResNet.py: Conv3d: weight bf = {weight.size()}')
        ...
        # print(f'ConResNet.py: Conv3d: x size = {x.size()}, weight = {weight.size()}')
...
def ConResNet_sobel(shape, num_classes=1, weight_std=True):

    # model = conresnet(shape, block=NoBottleneck, layers=[1, 2, 2, 2, 2], num_classes=num_classes, weight_std=weight_std)
