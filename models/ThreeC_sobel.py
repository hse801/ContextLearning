...
class AttBlock(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1),
                 dilation=(1, 1, 1), bias=False, weight_std=False, first_layer=False, num_groups=8):
        ...
        self.gn_seg = nn.GroupNorm(num_groups, in_planes)
        ...
        self.gn_res = nn.GroupNorm(num_groups, out_planes)
        self.conv_res = get_conv(out_planes, out_planes, kernel_size=(1, 1, 1),
                                 stride=(1, 1, 1), padding=(0,0,0),
                                 dilation=(dilation[0], dilation[1], dilation[2]), bias=bias, weight_std=self.weight_std)

        self.gn_res1 = nn.GroupNorm(num_groups, out_planes)
...
class ConvBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=(1, 1, 1), dilation=(1, 1, 1), downsample=None, fist_dilation=1,
                 multi_grid=1, weight_std=False, num_groups=8):
        super(ConvBlock, self).__init__()
        self.weight_std = weight_std
        self.relu = nn.ReLU(inplace=True)

        self.gn1 = nn.GroupNorm(num_groups, inplanes)
...
class ASPP(nn.Module):
    ...
    """
    def __init__(self, in_dims, out_dims, rate=[6, 12, 18], num_groups=8):
...
class CoConNet(nn.Module):
    def __init__(self, shape, block, layers, num_filters=16, in_channels=1, num_classes=2, weight_std=False, num_groups=8):
        ...
        self.encoder1 = nn.Sequential(
            # 32 -> 64
            nn.GroupNorm(num_groups, self.num_filters),
        ...
        self.encoder2 = nn.Sequential(
            # 64 -> 128
            nn.GroupNorm(num_groups, self.num_filters * 2),
        ...
        self.encoder3 = nn.Sequential(
            # 128 -> 256
            nn.GroupNorm(num_groups, self.num_filters * 4),
        ...
        self.fusionConv = nn.Sequential(
            nn.GroupNorm(num_groups, self.num_filters * 8),
            nn.ReLU(inplace=True),
            nn.Dropout3d(0.1),
    ...
    def _make_layer(self, block, inplanes, outplanes, blocks, stride=(1, 1, 1), dilation=(1, 1, 1), multi_grid=1, num_groups=8):
        downsample = None
    ...
    def forward(self, x_list):
        ...
        edge1 = torch.sigmoid(skip1)
        edge1 = self._sobel_torch(edge1)

        skip1 = skip1 * (1 + torch.sigmoid(edge1))
        ...
        edge2 = torch.sigmoid(skip2)
        edge2 = self._sobel_torch(edge2)

        skip2 = skip2 * (1 + torch.sigmoid(edge2))
        ...
        edge3 = torch.sigmoid(skip3)
        edge3 = self._sobel_torch(edge3)

        skip3 = skip3 * (1 + torch.sigmoid(edge3))
...
def ThreeC_sobel(shape, num_classes=2, weight_std=True):
    """
    Add edge attention to the skip connection
