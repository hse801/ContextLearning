...
class ASPP(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f'output shape = {self.output(out).size()}')
...
class CoConNet(nn.Module):
    def __init__(self, shape, block, layers, num_filters=24, in_channels=1, num_classes=2, weight_std=False):
        ...
        self.aspp3 = nn.Sequential(
            ASPP(self.num_filters * 2, self.num_filters * 2)
        )
...
def Co_Con_2ASPP(shape, num_classes=2, weight_std=True):
