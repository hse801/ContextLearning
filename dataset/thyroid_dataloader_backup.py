...
class Thyroid_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.test_flag == 0:
            ...
            return torch.FloatTensor(img_ct_data.copy()).unsqueeze(0), torch.FloatTensor(ct_res.copy()).unsqueeze(0), \
...
res_train_ds = Thyroid_dataset(crop_ct_path[60:368], mask_path=crop_mask_path[60:368], test_flag=0)
res_val_ds = Thyroid_dataset(crop_ct_path[0:60], mask_path=crop_mask_path[0:60], test_flag=1)

def generate_thyroid_dataset():

    # train_loader = DataLoader(train_ds, batch_size=2, num_workers=4)
    # val_loader = DataLoader(val_ds, batch_size=2, num_workers=4)
    train_loader = DataLoader(res_train_ds, batch_size=2, num_workers=0)
    val_loader = DataLoader(res_val_ds, batch_size=2, num_workers=0)
